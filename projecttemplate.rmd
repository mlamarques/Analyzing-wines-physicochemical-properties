...
This plot shows the inverse correlation between density and alcohol and a good color distinguish where most of the best wines are above 11% alcohol and 0.993 density.
...
In the second Multivariate plot, we can see the density and residual.sugar correlation. There is a nice distinction of colors, separated by the smooth line. Most higher quality wines are below the line, thus we might argue that testers prefer a higher residual.sugar/density ratio.
...
This plot does not add much value for the analysis. The smoth line doesn't have a good slope and the colors are randomly distributed, with no pattern.
...
Here we can see that there is a interval in the Citric Acid Above 0.25 and below 0.5 where the wine quality is considerably higher and the pH doesn't interfere so much.
...
Some correlation is notible between these two variables. We can distinguish an area where there is a concentration of most green dots, below the smooth line and 25 < free.sulfur.dioxide < 50, that are also below the smooth line, wich indicates a preference for a higher free.sulfur.dioxide/total.sulfur.dioxide ratio. Maybe there is some stage in the process of winemaking that directs affects this.
...
In this plot, the correlation between pH and alcohol is almost inexisting, and the quality is guided mainly by the alcohol quantity.
...
There is no clear indication of any of these variables having a relevant influence in the wine quality.
